...
function getPayInstance() {
  ...
  if (!mchid) return null

  payInstance = new WxPay({
    appid: process.env.WECHAT_APP_ID || '',  // Native 支付时可以传空，但 SDK 可能校验
    mchid,
...
export async function createNativeOrder({ description, outTradeNo, total, notifyUrl }) {
  ...
  try {
    ...
    if (result.status === 200 || result.status === 204) {
      return { code_url: result.data.code_url }
    }

    throw new Error(result.data?.message || '创建订单失败')
  } catch (err) {
    console.error('Wechat native order error:', err)
    throw err
  }
}
